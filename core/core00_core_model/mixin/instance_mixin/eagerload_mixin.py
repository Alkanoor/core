...
class EagerloadMixin(SessionMixin, IntrospectionMixin):
    ...
    @classmethod
    def _default_join_internal(cls, max_depth: int, current_depth: int,
        ...
        for key in relation_keys:
            if key[:2] != '__':  # otherwise these are indicative backref that would lead to infinite back & forth
                ...
                if not default_check_joinable() or hasattr(column_path.prop.argument, 'join'):
                    resolved, more_to_query = column_path.prop.argument.join(max_depth, current_depth+1, next_join_path)
    ...
    @classmethod
    def join(cls, max_depth: int = -1, current_depth: int = 0,
        ...
        def join_dispatch(joinable_obj):
            match joinable_obj['behavior']:
                case SQLJoinBehavior.EAGER_ALL:
                    return cls._default_join_internal(max_depth, current_depth, join_path,
                ...
                case SQLJoinBehavior.MAX_DEPTH:
                    ...
                    return cls._default_join_internal(new_max_depth, current_depth, join_path,
                                                      joinable_obj.get('join_on', None))
                ...
                case SQLJoinBehavior.CUSTOM:
                    return getattr(cls, joinable_obj['argument'])(max_depth, current_depth, join_path)
                case SQLJoinBehavior.NO_LOAD:
                    return cls._default_join_internal(max_depth, -1, join_path,
                                                      joinable_obj.get('join_on', None))
                case _:
